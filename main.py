from generator.comparison.generator import ComparisonGenerator
from generator.control.generator import ControlGenerator
...
def main(): 
    ...
    for question in test_questions_list:
        ...
        comparison_generator = ComparisonGenerator(llm, prompt)
        data_comparison = comparison_generator.generate(question)
        # 2-2. 질문을 가지고 generator/control 그룹에서 데이터를 생성함. 
        control_generator = ControlGenerator(llm, prompt)
        data_control = control_generator.generate(question)

    # 3. 생성된 데이터를 evaluator에서 평가함. 
    evaluator = Evaluator(llm, prompt)
